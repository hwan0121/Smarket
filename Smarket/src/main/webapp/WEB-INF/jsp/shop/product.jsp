...
<body>
	...
	<div class="container" style="width: 100%;">
		...
		<div class="abcc">
			...
			<span style="margin-left: 10px; font-size: 20px;"><b>${p.title}</b></span> 
			<br /> <span style="margin-left: 10px; font-size: 16px;">${p.category.name}</span>
			<br/>
			<span style="margin-left: 10px; font-size: 16px;"><fmt:formatDate
					value="${p.writedate}" pattern="yyyy.MM.dd HH:mm:ss" /></span> <br /> <span
				style="margin-left: 10px; color: orange; font-size: 18px;">${p.price}
				원</span><br /> <span style="margin-left: 10px; font-size: 18px;"><b>${p.detail}</b></span>
			<br />
		
			<label style="margin-left: 10px; font-size: 18px;">조회수</label>
			<span style="height: 15px; margin-left: 5px; font-size: 20px;">${p.click}</span>
			<br/>
			<label style="margin-left: 10px; font-size: 18px;">관심</label>
			<span style="height: 15px; margin-left: 5px; font-size: 20px;">${pcnt}</span>
			<br/>
			<label style="margin-left: 10px; font-size: 18px;">댓글</label>
			...
			<!--  댓글  -->
			<div class="container" style="display: flex; align-items: center; margin-bottom: 30px;">
				<label for="content" style="margin-top: 32.5px;">comment</label>
				<form name="commentInsertForm">
					<div class="input-group">
						<input type="hidden" name="pid" value="${p.id}" /> <input
							type="text" class="form-control w400" id="content" name="content" style="margin-top:30px"
							placeholder="내용을 입력하세요."> <span class="input-group-btn">
							<sec:authorize access="authenticated">
								<sec:authentication property="user.id" var="currentid" />
								<input type="hidden" name="uid" value="${currentid}" />
							</sec:authorize>
							<button class="btn btn-dark" type="button" style="margin-top: 32.5px;"
